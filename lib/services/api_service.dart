...
class ApiService {
  ...
  static const String baseUrl = "https://nillohitroy-cogniscript-medical-ai.hf.space";
  ...
  static Future<Map<String, dynamic>> analyzePrescription(File imageFile) async {
    try {
      ...
      if (postResponse.statusCode != 200) {
        throw Exception("Server Error on Submission: ${postResponse.statusCode}");
      }
      ...
      if (responseString.contains('event: complete')) {
        ...
        if (dataIndex != -1) {
          ...
          String cleanJson = aiOutputString.replaceAll('```json', '').replaceAll('```', '').trim();
